...
def find_angelina
  Actor.find_by(name: "Angelina Jolie")  # Actor.select(Actor.*).where(name: "Angelina Jolie")
end
...
def below_average_years
  ...
  Movie.select("yr, count(*) as bad_movies").where(score: 0...5).group(:yr)
...
def pulp_fiction_actors
  ...
  Actor.select(:id, :name).joins(:castings).joins(:movies).where("movies.title = 'Pulp Fiction'").distinct
end
...
def uma_movies
  ...
  Movie.select(:id, :title, :yr).joins(:castings).joins(:actors).where('name = ?', 'Uma Thurman').order("movies.yr asc").distinct
